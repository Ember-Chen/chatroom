...
public class WsServer {
    ...
    public void onMessage(String message, Session session, @PathParam("username") String username){
        ...
        } else if(msgType.equals("side_txt")){ // 转发私聊消息
            sendOneMsg(username,target,content,msgType);
        } else if(msgType.equals("group_txt")){ // 转发群聊消息
            sendGroupMsg(username,groupId,content,msgType);
        } else if(msgType.equals("exile")&&username.equals("ADMIN")){ // 踢人
            exile(target);
        } else if(msgType.equals("dismiss")&&username.equals("ADMIN")){ // 解散群组
            dismissGroup(groupId);
        } else if(msgType.equals("get_group_info")){ // 发送群组信息
            sendGroupInfo(groupId,username);
        } else if(msgType.equals("update_user")){ // 更新用户列表请求
            sendOneMsg("SERVER",username,userList(),"update_user");
        } else if(msgType.equals("update_group")){ // 更新群组列表请求
            sendOneMsg("SERVER",username, groupList(),"update_group");
        }
        else {
...
//        if(username.equals("ADMIN")) return;
    ...
    public static void dismissGroup(String group){
        Set<String> usrOfGroup = groupMap.get(group);
        groupMap.remove(group);
        // 向群组成员群发"解散群聊"的通知
        if(usrOfGroup != null){
            for(String usr : usrOfGroup) {
                sendOneMsg("SERVER", usr, group, "dismiss_group");
            }
        }
        // 向ADMIN发送更新后的群组列表
        sendOneMsg("SERVER", "ADMIN", groupList(), "update_group");
    }
